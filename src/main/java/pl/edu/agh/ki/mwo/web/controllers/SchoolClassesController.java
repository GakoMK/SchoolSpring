...
public class SchoolClassesController {
	...
	public String displayEditSchoolClassForm(
			@RequestParam(value = "schoolClassID", required = false) String schoolClassID, Model model,
			HttpSession session) {
		if (session.getAttribute("userLogin") == null) {
			return "redirect:/Login";
		}
	...
	@RequestMapping(value = "/ConfirmEditSchoolClass", method = RequestMethod.POST)
	public String editSchool(
		...
		SchoolClass schoolClass = DatabaseConnector.getInstance().getSchoolClassByID(classID);
